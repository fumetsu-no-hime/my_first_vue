...
<template>
  <section class="card gap-4 flex flex-col">
    ...
    <div class="flex justify-between items-strech">
      <div class="flex items-center w-2/4 border border-black">
        <button type="button" class="grow text-center" @click="minus()">-</button>
        <input v-model="num" type="number" class="w-2/3 border-y-0 outline-0 border-x-[1px] border-black focus:ring-0 text-center" @change="inputNum()">
        <button type="button" class="grow text-center" @click="plus()">+</button>
      </div>
      <button type="button" class="w-1/3 border bg-[#e3e3e3] rounded-[5px] text-[#686868] text-sm drop-shadow-lg" @click="addCart()">加入購物車</button>
    </div>
  </section>
</template>
